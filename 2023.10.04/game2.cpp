...
int f[101][50];
int able[50];
int dfs(bool limit,int sum,int pos) {
	if(!pos&&sum%n==0) return 1;
	if(!pos) return 0;
	if(!limit&&f[sum][pos]) return f[sum][pos];
	if(pos*9<n-(sum%n)) return f[sum][pos]=sum%n==0?1:0;
	int up=limit?able[pos]:9;
	int res=0;
	for(int i=0;i<=up;i++) res+=dfs(limit&(i==up),sum+i,pos-1);
	if(!limit) f[sum][pos]=res;
	return res;
}
int solve(int x) {
	int tot=0;
	...
	if(tot<=1) {
		int ans=0;
		for(int i=0;i<=x;i++) ans+=i%n==0;
		return ans;
	}
	int up=able[tot];
	int ans=0;
	for(int i=0;i<=up;i++) ans+=dfs(i==up,i,tot-1);
	return ans;
}
signed main() {
//	_File();
