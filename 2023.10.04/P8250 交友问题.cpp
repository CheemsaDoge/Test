...
//	_File();
	...
	for(int i=1;i<=q;i++) {
		...
		query[u].push_back(std::make_pair(v,i));
	}
	for(int i=1;i<=n;i++) {
		int lst=0,lstans=-1145;
		sort(query[i].begin(),query[i].end());
		for(int p : e[i]) vis[p]=i;
		for(pair<int,int>ltt : query[i]) {
			int res=0;
			const int v=ltt.first,id=ltt.second;
			if(v==lst) {ans[id]=-lstans;continue;} else lst=v;
			for(int p : e[v]) res+=vis[p]==i;res+=vis[v]==i;lstans=res;ans[id]-=res;
		}
	}
