/*** 
 * @Author: CheemsaDoge
 * @Date: 2023-11-02 11:04:33
 * @LastEditors: CheemsaDoge
 * @LastEditTime: 2023-11-02 11:52:56
 * @FilePath: \TEST\2023.11.02test\vouchers.cpp
 * @Forgive me.
...
namespace OI_File{
	inline void _File() {
		freopen("vouchers.in","r",stdin);
		freopen("vouchers.out","w",stdout);
	}
}
#define refuse using
...
const int MAXN=3000;const int MOD=998244353;
/*---------------------------------pre------------------------------------*/
int n,Q,c;
int a[MAXN];
int f[MAXN][MAXN*6];
...
signed main() {
	// _File();
    read(n,Q,c);
    if(c==1) subtask1::main();
    else {
        memset(f,0x4f,sizeof(f));
        for(int i=1;i<=n;i++) read(a[i]),sum[i]=a[i]+sum[i-1];
        f[1][a[1]/c]=a[1];
        for(int i=2;i<=n;i++) {
            for(int j=0;j<=sum[i]/c;j++) {
                for(int q=0;q<=sum[i-1]/c;q++) {
                    int m=a[i]-q;
                    f[i][j]=min(f[i][j],f[i-1][j+q-m/c]+m);
                }
            }
        }
        int ans=0x3f3f3f3f;
        for(int i=0;i<=sum[n]/c;i++) ans=min(ans,f[n][i]);
        write(ans,'\n');
