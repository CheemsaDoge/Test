...
const int MAXN=1.5e6;const int MOD=998244353;
/*---------------------------------pre------------------------------------*/

int n,num[MAXN],minn,maxn,id,ans;
signed main() {
	...
	for(int i=2;i<=n;++i){
		read(num[i]);
		minn=min(minn,num[i]);
		maxn=max(maxn,num[i]-minn);
	}
	for(int i=1,j,lef,ret;i<=n;i=j){
		for(j=i,lef=0,ret=0;j<=n&&num[j]>=num[i];++j)
			if(num[i]==num[j]) ++lef;
			else if(num[j]-num[i]==maxn) {
				--lef; 
				ret=min(ret,lef); 
				++ans; 
			}
