/*** 
 * @Author: CheemsaDoge
 * @Date: 2023-11-02 07:46:53
 * @LastEditors: CheemsaDoge
 * @LastEditTime: 2023-11-02 11:52:42
 * @FilePath: \TEST\2023.11.02test\sequence.cpp
 * @Forgive me.
...
std::multiset<LL>st;
LL maxx;
LL maxa[MAXN],mina[MAXN];
std::unordered_map<int,int>del;
std::unordered_map<int,int>dell;
int ans;
signed main() {
	_File();
    read(n);maxx=0;
    for(int i=0;i<=n+4;i++) mina[i]= 0x3f3f3f3f3f3f3f3f;
    for(int i=0;i<=n+4;i++) maxa[i]=-0x3f3f3f3f3f3f3f3f;
    for(int i=1;i<=n;i++) {
        read(a[i]);
        st.insert(a[i]);
    ...
    for(int i=n;i>1;i--) {
        ...
        maxx=max(maxx,maxa[i]-mina[i-1]);
    }
    for(int i=1;i<=n;i++) {
        if(del.find(a[i])!=del.end()&&del[a[i]]&&st.find(a[i]+maxx)==st.end()) {
            ans+=min(del[a[i]]-(LL)st.count(a[i]),dell[a[i]+maxx]);
            del[a[i]]=dell[a[i]+maxx]=0;
        }
        if(dell.find(a[i])!=dell.end()&&dell[a[i]]&&st.find(a[i]-maxx)==st.end()) {
            ans+=min(del[a[i]-maxx],dell[a[i]]);
            dell[a[i]]=del[a[i]-maxx]=0;
        }
        if(del[a[i]]==0&&st.find(a[i]+maxx)!=st.end()) {
           del[a[i]]=st.count(a[i]);
           dell[a[i]+maxx]=st.count(a[i]+maxx);
        } 
