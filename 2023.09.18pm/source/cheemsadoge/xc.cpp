...
const int MAXM=1e6+1145;const int MAXN=1e3+11145;const int INF=2147483647ll;//2^31-1
#define ll long long
#define pc putchar('\n')
#define pk putchar(' ')
/*---------------------------------pre------------------------------------*/
...
//	_File();
	...
	for(int i=1;i<=n;i++) for(int j=1;j<=n;j++) dis[i][j]=INF,read(map[i][j]);
	ans=(n*2-2)/k*(A+C);
	dfs(1,1,0,k,0);
	write(ans);
	return 0;
