...
namespace OI_File{
	inline void _File() {
		freopen("trz.in","r",stdin);
		freopen("trz.out","w",stdout);
	}
}
...
signed main() {
	...
	for(int i=5;i<=1000;i++) {
		C[i][1]=i;C[i][0]=1;
		for(int j=2;j<=4;j++) C[i][j]=(C[i-1][j-1]+C[i-1][j])%MOD;
//		write(i,C[i][1],C[i][2],C[i][3],C[i][4],'\n');
	}
	int t;read(t);while(t--) {
		...
		for(int i=1;i<=n;i++) {
			if(deg[i]<6) continue;
			for(int v:e[i]) for(int k:e[v]) if(k!=i) to[i][k]++;
			for(int j=1;j<=n;j++) {
				if(to[i][j]<4||i==j) continue;
				ans+=C[deg[i]-4-map[i][j]][2]*C[to[i][j]][4]%MOD;
				ans%=MOD;
			}
		}
		write(ans%MOD,'\n');
