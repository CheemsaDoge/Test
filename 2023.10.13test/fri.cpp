...
LL sum,res;
LL n,m,a,b;
LL jie[MAXN],ni[MAXN],s1[MAXN],s2[MAXN],s3[MAXN];
inline LL C(int x,int y) {return jie[x]*ni[y]%MOD*ni[x-y]%MOD;}
signed main() {
	_File();
	jie[0]=ni[0]=1;
	for(int i=1;i<=MAXN-5;i++) jie[i]=jie[i-1]*i%MOD;
	ni[MAXN-5]=quick_pow(jie[MAXN-5],MOD-2);
	for(int i=MAXN-6;i>=1;i--) ni[i]=ni[i+1]*(i+1)%MOD;
	int t;read(t);while(t--) {
		read(n,m,a,b);s1[0]=s2[0]=s3[0]=1;
		for(int i=1;i<=n;i++) s1[i]=s1[i-1]*a%MOD;
		for(int i=1;i<=n;i++) s2[i]=s2[i-1]*b%MOD; 
		for(int i=1;i<=n;i++) s3[i]=s3[i-1]*(b-a)%MOD;
		s2[n]=quick_pow(s2[n],MOD-2);sum=res=0;
		for(int i=1;i<=n;i++) {
			LL ans=s2[n]*s1[i]%MOD*s3[n-i]%MOD*C(n,i)%MOD;
			sum=(sum+quick_pow(i,m))%MOD;
			res=(res+sum*ans%MOD)%MOD;
		}
		write(res%MOD,'\n');
	}
	return 0;
