...
#define pn puts("NO")
#define py puts("YES")
const int MAXN=1.5e5;const int MAXM=12.5e5;
/*---------------------------------pre------------------------------------*/
int s,t,d;
bool oil[MAXN];
int vis1[MAXN];
...
struct disf {
	int lst,v;
	bool f;
	bool operator <(const disf &b) const {return v<b.v;}
};
std::priority_queue<disf>que;
int n,m,k,q,tmp;
bool flag;
void bfs() {
	que.push((disf){d,s,false});
	que.push((disf){d,t,true});
//	write('\n',s,t,'s','\n');
	while(!que.empty()) {
		disf u=que.top();que.pop();
//		write('u',u.v,u.lst,u.f,'\n');
		if(u.f) {
			if(vis1[u.v]&&vis2[u.v]>=d-vis1[u.v]) return flag=true,void();
			if(u.lst>vis2[u.v]) vis2[u.v]=oil[u.v]?d:u.lst;
			else continue;
		}
		else {
			if(vis2[u.v]&&vis1[u.v]>=d-vis2[u.v]) return flag=true,void();
			if(u.lst>vis1[u.v]) vis1[u.v]=oil[u.v]?d:u.lst;
			else continue;
		}
		for(Edge v:e[u.v]) {
//			write('e',v.v,v.w,'\n');
			if(u.lst<v.w) break;
			que.push((disf){u.lst-v.w,v.v,u.f});
		}
	}
}
...
signed main() {
	_File();
	read(n,m,k,q);
	for(int i=1;i<=k;i++) read(tmp),oil[tmp]=true;
	for(int i=1;i<=m;i++) {
