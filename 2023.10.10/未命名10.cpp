...
int main() {
	read(n);
    for(int i=1;i<=n;++i) read(t),a.insert(t+a.begin(),i);
    for(int i=0;i<n;++i) t=a[i],update(t,ans[t]=query(t)+1);
    for(int i=1;i<=n;++i) write(ans[i]=max(ans[i],ans[i-1]),'\n');
    return 0;
}
