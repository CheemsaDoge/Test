...
namespace CheemsaDoge {
	#define range(a,b) int i=a;i<=n;i++
...
namespace OI_File{
	inline void _File() {
		freopen("airen.in","r",stdin);
		freopen("airen.out","w",stdout);
	}
}
...
struct airen {
	int a,b,need;
	bool operator <(const airen &s) const {return need>s.need;}//?true:a>s.a;
}ps[MAXN];
int f[MAXN],sum[MAXN];
...
signed main() {
	_File();
	read(n);
	for(int i=1;i<=n;i++) read(ps[i].a,ps[i].b);	
	read(h);
	for(int i=n;i>=1;i--) ps[i].need=h-ps[i].a-ps[i].b;
	sort(ps+1,ps+n+1);
	for(int i=n;i>=1;i--) sum[i]=sum[i+1]+ps[i+1].a;
	LL tot=0;
	For(i,1,n,1) {
		if(tot+sum[i]>=ps[i].need) {
			que.push(ps[i].a);
			ans++;
		} else if(!que.empty()&&que.top()>ps[i].a) {
			tot+=que.top();
			que.pop();
			que.push(ps[i].a);
		} else tot+=ps[i].a;
