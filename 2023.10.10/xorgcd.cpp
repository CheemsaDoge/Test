...
const int MAXN=2e5;const int MAXT=12000;
/*---------------------------------pre------------------------------------*/
int maxx;
int ans[30000009],q[MAXT];
int cnt=0;
signed main() {
	_File();
	int n,t;read(t);
	if(t>=2) {
		...
		for(int i=1;i<=maxx/2;i++) for(int j=2;i*j<=maxx;j++) {
			b=i*j;
			if((b^i)==(b-i)) ans[b]++;	//（约莫340ms （对于T=10000，every n = 3e7 
//			ans[b]+=(b^1)==(b-i); 用这个的实测时间为上面一句的10倍（约莫3600ms （De了我1小时，还以为复杂度算错了 
		}
		for(int i=1;i<=maxx;i++) ans[i]+=ans[i-1];
		for(int i=1;i<=t;i++) std::printf("Case %d: %d\n",i,ans[q[i]]);
		return 0;
	}
	else {
		for(int w=1;w<=t;w++) {
			read(n);int res=0;
			for(int i=1;i<=n/2;i++) for(int j=2;i*j<=n;j++) res+=((j*i)^i)==((j-1)*i);
			std::printf("Case %d: %d\n",w,res);
		}
	}
	return 0;
