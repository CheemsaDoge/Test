...
/*---------------------------------pre------------------------------------*/
int cnt[20],n,m,y,ans,player[MAXN];
int cs(int x) {return __builtin_popcount(x);}
inline void makepair(int s) {
	int t=min(cnt[s],cnt[s^y]);
	ans+=t;
	cnt[s]-=t;cnt[s^y]-=t;
}
signed main() {
	...
	for(int i=0;i<m;i++) {
		...
		for(int j=0;j<why;j++) {
			int tmp;read(tmp);
			player[tmp]|=(1<<i);
//			write('s',i,tmp,player[tmp],'\n');
		}
	}
	for(int i=1;i<=n;i++) cnt[player[i]]++;
//	for(int i=0;i<=y;i++) write(i,cnt[i],'\n');
	for(int s=0;s<=y;s++) if(cs(s)>1) makepair(s);
	for(int s=0;s<=y;s++) if(cs(s)>2) ans+=cnt[s];
	...
	for(int k=0;k<m;k++) for(int i=0;i<m;i++) for(int j=i+1;j<m;j++) {
