...
const int MAXN=1.1e6;const int MOD=998244353;
/*---------------------------------pre------------------------------------*/
struct Edge {
	int u,v;
}edge[MAXN];
...
bool deg[MAXN];
inline LL LLrand() {
	return rand()*rand()%sizeof(LL)*rand()%sizeof(LL);
}
signed main() {
	_File();
	srand(time(0)*time(0)%14235*time(0)%12432*time(0));
	read(n,m);
	int t=m;
	for(int i=1;i<=m;i++) {
		read(edge[i].u,edge[i].v);
		deg[edge[i].u]^=1,deg[edge[i].v]^=1;
	}
	for(LL i=m;i>=1;i--) {
		if(m==1||m==0) break;
		if((!deg[edge[i].u])&&(!deg[edge[i].v])) delet[i]=true,deg[edge[i].u]^=1,deg[edge[i].v]^=1,m-=2;
		else if((deg[edge[i].u]||deg[edge[i].v])&&LLrand()%(m*m*m)+1<=i*i*i) delet[i]=true,deg[edge[i].u]^=1,deg[edge[i].v]^=1,m--;
	}
	for(int i=1;i<=t;i++) write((int)!delet[i]);
	return 0;
}
