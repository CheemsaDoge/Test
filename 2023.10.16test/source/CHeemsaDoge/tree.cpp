...
signed main() {
	_File();
	read(a,b);
	if(a==0&&b==0) return write(1),0;
	if(a<b+2) return write(0),0;
	if(a==b+3) return write(0),0;
	if(a==b+2) {
		write(a+b,'\n');
		for(int i=1;i<=b;i++) write(cnt,cnt+1,'\n'),cnt++;
		for(int i=1;i<=b;i++) write(i,++cnt,'\n');
		write(1,++cnt);
	}
	else {
		write(a+b+1,'\n');
		...
		for(int i=1;i<=b;i++) write(i,++cnt,'\n');
		write(1,++cnt,'\n');LL q=cnt;
		for(int i=1;i<=a-b-1;i++) write(q,++cnt,'\n');
	}
	return 0;
}
