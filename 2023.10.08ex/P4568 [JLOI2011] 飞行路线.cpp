...
void Dijkstra(int s) {
	...
	dis[s]=0;
	std::priority_queue<pair<int,int>,vector<pair<int,int> >,std::greater<pair<int,int> > > points;
	points.push(std::make_pair(0,s));
	while(!points.empty()) {
		...
		if(!vis[u]) {
			...
			for(int i=head[u];i;i=edge[i].next) {
				...
				if(dis[to]>dis[u]+edge[i].cost) {
					...
					points.push(std::make_pair(dis[to],to));
				}
			}
		}
	}
...
signed main() {
	...
	for(int i=1;i<=k;++i) add_edge(t+(i-1)*n,t+i*n);
	Dijkstra(s);
	write(dis[t+k*n]);
	return 0;
}
