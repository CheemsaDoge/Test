 ...
 * @LastEditTime: 2023-11-09 18:59:37
 * @FilePath: \TEST\数列分块\数列分块入门6.cpp
 * @Forgive me.
...
const int MAXN=6.5e4+5;const int MAXB=520;const int MOD=998244353;
/*---------------------------------pre------------------------------------*/
int head,totr;
struct Part {
    int num,nxt,pre;
...
inline void split(int x) {
    block[++totr].nxt=block[x].nxt;
    block[x].nxt=totr;
    block[totr].pre=x;
    int nw=0;
    for(int i=block[x].num/2;i<block[x].num;i++) nw++,block[totr].w.emplace_back(block[x].w[i]);
    for(int i=block[x].num/2;i<block[x].num;i++) block[x].w.pop_back();
    block[x].num-=nw;block[totr].num=nw;
}
