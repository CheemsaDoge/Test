 ...
 * @LastEditTime: 2023-11-09 10:23:53
 * @FilePath: \TEST\数列分块\数列分块入门5.cpp
 * @Forgive me.
...
const int MAXN=6.5e4+5;const int MAXB=450;const int MOD=998244353;
/*---------------------------------pre------------------------------------*/
int bl;
int L[MAXB],R[MAXB],belong[MAXN];
int tag[MAXB],a[MAXN],b[MAXN],sum[MAXB],maxn[MAXB];
int totr=1;
inline void sqrts(int &x,int n) {
    while(n--) x=sqrt(x);
    return void();
}
inline void AreyougoingtoScabroughFairParsleysagerosemaryandthymeRemembermetoonewholivesthereHeoncewasthetrueloveofmine(int x) {
    if(maxn[x]<=1) return void();
    if(tag[x]==0) {
        maxn[x]=0;sum[x]=0;
        for(int i=L[x];i<=R[x];i++) sum[x]+=a[i],maxn[x]=max(maxn[x],a[i]);
        return void();
    } 
    if(tag[x]>=7) {
...
inline void modify(int l,int r) {
    ...
    if(ls==rs) {
        ...
        AreyougoingtoScabroughFairParsleysagerosemaryandthymeRemembermetoonewholivesthereHeoncewasthetrueloveofmine(ls);
        return void();
    }
    for(int i=l;i<=R[ls];i++) a[i]=sqrt(a[i]);
    for(int i=L[rs];i<=r;i++) a[i]=sqrt(a[i]);
    for(int i=ls+1;i!=rs;i++) tag[i]+=1;
    AreyougoingtoScabroughFairParsleysagerosemaryandthymeRemembermetoonewholivesthereHeoncewasthetrueloveofmine(ls);
    AreyougoingtoScabroughFairParsleysagerosemaryandthymeRemembermetoonewholivesthereHeoncewasthetrueloveofmine(rs);
    return void();
}
inline int query(int l,int r) {
    ...
    for(int i=l;i<=R[ls];i++) ans=(ans+a[i]);
    for(int i=L[rs];i<=r;i++) ans=(ans+a[i]);
    for(int i=ls+1;i!=rs;i++) {
        AreyougoingtoScabroughFairParsleysagerosemaryandthymeRemembermetoonewholivesthereHeoncewasthetrueloveofmine(i);
        ans=(ans+sum[i]);
    }
    return ans;
}
