...
const int MAXM=1e5+10;const int MAXN=3e5+10;const int INF=2147483647ll;//2^31-1
#define ll long long
#define pc putchar('\n')
#define pk putchar(' ')
/*---------------------------------pre------------------------------------*/
...
int main() {
	...
	int ln=log2(n);
	for(int i=1;i<=ln;i++) {
		for(int j=1;j+(1<<i)<=1+n;j++) stmax[j][i]=max(stmax[j][i-1],stmax[j+(1<<(i-1))][i-1]);
	}
	read(m);
	for(int i=1;i<=m;i++) {
		int a,b;read(a,b);
		ln=log2((b-a+1));
		write(max(stmax[a][ln],stmax[b-(1<<(ln))+1][ln]));pc;
	}
	return 0;
}
