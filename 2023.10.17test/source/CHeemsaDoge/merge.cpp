...
/*---------------------------------pre------------------------------------*/
int n,w;
LL line[MAXN],vis[MAXN],ans=0;
signed main() {
	_File();
	int T;read(T);while(T--) {
		read(n,w);ans=0;memset(vis,0,sizeof(vis));
		for(int i=1;i<=n;i++) read(line[i]);
		for(LL i=w-1;i>=0;i--) {
			LL Up=0,Down=1ll<<61;
			for(int j=1;j<=n;j++) {
			   LL now=((line[j]>>i)|vis[j]);
			   Up+=1ll<<(61-(int)floor(log2((~now)&(-~now))));
			   if(Up>=Down) break;
			}
			if(Up>=Down) for(int j=1;j<=n;j++) vis[j]|=line[j]>>i;
			else ans+=1ll<<i;
		}
		write(ans,'\n');
	}
