...
//	_File();
	...
	sort(a+1,a+1+n);
	for(int i=1;i<=n;i++) {
		for(int j=1;j<=__builtin_sqrt(a[i]);j++) {
			if(a[i]%j) continue;
			if(j<=a[1]) hash[j]=__gcd(hash[j],a[i]);
			if(a[i]/j<=a[1]) hash[a[i]/j]=__gcd(hash[a[i]/j],a[i]);
		}
	}
	int ans=0;
	for(std::pair<int,int>v:hash) if(v.first==v.second) ans++;
	write(ans);
	return 0;
}
