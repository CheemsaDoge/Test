 ...
 * @LastEditTime: 2023-11-09 14:49:03
 * @FilePath: \TEST\2023.11.09test\[POI2004] PRZ.cpp
 * @Forgive me.
...
int tm[29],wh[29];
int f[131072];
int C[131072],T[131072];
signed main() {
	// _File();
    read(W,n);
    for(int i=1;i<=n;i++) read(tm[i],wh[i]);
    memset(f,0x3f,sizeof(f));
    f[0]=0;
    int sum=(1<<n)-1;
    for(int i=0;i<=sum;i++) for(int j=1;j<=n;j++) if(i&(1<<(j-1))) T[i]=max(tm[j],T[i]),C[i]+=wh[j];
    for(int i=0;i<=sum;i++) {
