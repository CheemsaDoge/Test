 ...
 * @LastEditTime: 2023-11-09 16:23:39
 * @FilePath: \TEST\2023.11.09test\[SCOI2007] 排列.cpp
 * @Forgive me.
...
/*------------------------------------pre------------------------------------*/
int T,d,len,a[22],cnt,dp[1<<16][1002];
bool b[22];
char s[22];
int main() {
    read(T);while(T--) {
		...
		for(int S=0;S<(1<<len)-1;S++) {
			...
			for(int j=1;j<=len;j++)
                if(!(S&(1<<(j-1)))&&!b[a[j]]) {
				    b[a[j]]=1;
				    for(int k=0;k<d;k++) dp[S|(1<<(j-1))][(k*10+a[j])%d]+=dp[S][k];
			    }
		}
		write(dp[(1<<len)-1][0],'\n');
	}
