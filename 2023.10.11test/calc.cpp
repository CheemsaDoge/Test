...
{
	/*initialize();*/b%=p;std::map<int,int>hash;hash.clear();hash[b]=0;int cfa=1;int t=int(sqrt(p))+1;
...
int y,z,p;
signed main() {
	_File();int xx,yy,T,k;read(T);read(k);while(T--) {
		read(y,z,p);
		if(k==1) write(fast_pow(y,z,p),'\n');
		else if(k==3) {
			int ans=BsGs(y,z,p);
			if(ans==-1) pn;
			else write(ans,'\n');
			...
			if(z%gcd) pn;
				...
				int temp=p/gcd;
				while(xx<0) xx+=temp;
				write(((1ll*xx*z/gcd)%temp+temp)%temp,'\n');
			}
		}
	}
	return 0;
