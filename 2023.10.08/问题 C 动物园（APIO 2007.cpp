...
/*---------------------------------pre------------------------------------*/
int n,m,ans;
int dp[MAXN][50],num[MAXN][50];
signed main() {
	read(n);read(m);
	for(int i=1,x,y,z,cdy,wzy;i<=m;++i) {
		cdy=wzy=0;
		read(x);read(y);read(z);
		for(int j=1,tmp;j<=y;++j) {
			read(tmp);
			tmp=(tmp-x+n)%n;
			cdy|=(1<<tmp);
		}
		for(int j=1,tmp;j<=z;++j) {
			read(tmp);
			tmp=(tmp-x+n)%n;
			wzy|=(1<<tmp);
		}
		for(int j=0;j<32;++j) if((cdy&j)||((~j)&wzy)) num[x][j]++;
	}
	for(int x=0;x<32;++x) {
		memset(dp[0],-0x3f,sizeof dp[0]);
		dp[0][x]=0;
		for(int i=1;i<=n;++i) for(int j=0;j<32;++j) dp[i][j]=max(dp[i-1][(j&15)<<1],dp[i-1][(j&15)<<1|1])+num[i][j];
		ans=max(ans,dp[n][x]);  
	}
   	write(ans);
    return 0;
