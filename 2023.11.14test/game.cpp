 ...
 * @LastEditTime: 2023-11-14 15:40:57
 * @FilePath: \TEST\2023.11.14test\game.cpp
 * @Forgive me.
...
const int MAXN=1e5+6;const int MOD=998244353;
/*---------------------------------pre------------------------------------*/
struct node {
	int v,id,rk,calc;
	const bool operator <(const node&xx) const {return v==xx.v?id<xx.id:v>xx.v;}
}a[MAXN];
int n,q;
bool vis[MAXN];
signed main() {
	...
	read(n,q);
	for(int i(1);i <= n;++i){
		read(a[i].v);a[i].id = i,a[i].rk = 0,a[i].calc = 0;
	}
	sort(a+1,a+n+1);
	while(q--) {
		int p;read(p);
		for(int i=1;i<=n;++i) {
			if(a[i].id<=p) a[i].rk=0;
			else a[i].rk=a[i].id-p;
		}
		int cnt = 1;
		for(int i(1);i <= n;++i) {
			if(a[i].rk<cnt) {
		...
		LL tot=0;
		for(int i=1;i<=n;++i) tot+=a[i].calc%2?a[i].v:-a[i].v;
