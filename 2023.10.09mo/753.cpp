...
int f[MAXN][3][3][3][3];
int a[421];
int dfs(int limit,int pos,int lead,int w,int b,int c) {
	LL ans=0;
	if(pos==0) return w&&b&&c&&lead;
	if(!limit&&f[pos][lead][w][b][c]) return f[pos][lead][w][b][c];
	if(!limit||a[pos]>=3) ans+=dfs((a[pos]==3)&&limit,pos-1,1,1,b,c);
	if(!limit||a[pos]>=5) ans+=dfs((a[pos]==5)&&limit,pos-1,1,w,1,c);
	if(!limit||a[pos]>=7) ans+=dfs((a[pos]==7)&&limit,pos-1,1,w,b,1);
	if(!lead) ans+=dfs(0,pos-1,0,0,0,0);
	return f[pos][lead][w][b][c]=ans;
}
...
//	_File();
	...
	LL ans=0;
	if(a[cnt]>=3) ans+=dfs((a[cnt]==3),cnt-1,1,1,0,0);
	if(a[cnt]>=5) ans+=dfs((a[cnt]==5),cnt-1,1,0,1,0);
	if(a[cnt]>=7) ans+=dfs((a[cnt]==7),cnt-1,1,0,0,1);
	ans+=dfs(0,cnt-1,0,0,0,0);	
	write(ans);
	return 0;
}
