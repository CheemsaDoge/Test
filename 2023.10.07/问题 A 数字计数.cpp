...
LL dfs(int lead,int limit,int pos,int as,int ask) {
	if(pos<=0) return as;
	if(f[pos][as][limit][lead]) return f[pos][as][limit][lead];
	LL ans=0;
	int up=limit?a[pos]:9;
	for(int i=0;i<=up;i++) {
		if(i==0&&(lead)) ans+=dfs(1,limit&&(i==up),pos-1,as,ask);
		else ans+=dfs(0,limit&&(i==up),pos-1,as+(ask==i),ask);
	}
	return f[pos][as][limit][lead]=ans;
}//ask==0?i==0?lead?1:0:0:
LL solve(LL x,int num) {
	int totr=calc(x);
	LL ans=0;//x
	memset(f,0,sizeof(f));
	if(totr<=1) return num==0?0:num<=x;
	int up=a[totr];
	for(int i=0;i<=up;i++) ans+=dfs(!i,up==i,totr-1,num==0?0:i==num,num);
	return ans;
}
...
//	_File();
	LL l,r;
	read(l,r);
	for(int i=0;i<=9;i++) write(solve(r,i)-solve(l-1,i)),putchar(' ');
	return 0;
}
