...
vector<int>po;
bool vis[MAXN];
void dfs1(int dep,int fa,int x,bool t=0) {
	if(t) if(dep==max_dep) po.push_back(x);
	if(t) if(dep>max_dep) po.clear(),po.push_back(x),max_dep=dep;
	if(!t) if(dep>max_dep) max_dep=dep;
	for(int v:e[x]) {
		if(v==fa) continue;
		dfs1(dep+1,x,v,t);
	}
...
bool dfs2(int u,int fa,int dep) {
	if(vis[u]) return bool(true);
	bool allright=0;
	if(dep==max_dep) allright=bool(true);
	for(int v:e[u]) {
		if(v==fa) continue;
		if(dfs2(v,u,dep+1)) allright=bool(true);
	}
	if(!vis[u]&&allright) vis[u]=1,que.push(-u);
	return bool(allright);
}
signed main() {
//	_File();
	read(n);
	...
	dfs1(1,-1,1,1);
	dfs1(1,-1,po[0]);
	for(int v:po) dfs2(v,-1,1);
	while(!que.empty()) write(-que.top()),que.pop(),putchar('\n');
	return 0;
}
