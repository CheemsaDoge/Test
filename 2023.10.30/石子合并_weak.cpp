/*
 * @Author: CheemsaDoge
 * @Date: 2023-10-29 20:58:12
 * @LastEditors: CheemsaDoge
 * @LastEditTime: 2023-10-30 09:31:17
 * @FilePath: \TEST\2023.10.29\石子合并.cpp
...
const int MAXN=400;const int MOD=998244353;
/*---------------------------------pre------------------------------------*/
int n,m;
int dp[MAXN][MAXN];
int sum[MAXN];
signed main() {
	...
	for(int i=1;i<=n;i++) read(sum[i]),sum[i]+=sum[i-1],dp[i][i]=0;
