/*** 
 * @Author: CheemsaDoge
 * @Date: 2023-11-15 14:50:58
 * @LastEditors: CheemsaDoge
 * @LastEditTime: 2023-11-15 15:42:42
 * @FilePath: \TEST\2023.11.15ex\matrix.cpp
 * @Forgive me.
...
int hc,lc;
int a[1145][1919];
LL h[1005000],l[1000500];
namespace subtask1 {
	int ans;
	std::priority_queue<LL>hang;
	std::priority_queue<LL>lie;
	signed main() {
		lc=hc=0;
		for(int i=1;i<=n;i++) {
			LL sum=0;
        ...
        for(int i=1;i<=k;i++) {
            l[i]=l[i-1]+lie.top();
            h[i]=h[i-1]+hang.top();
            LL t=lie.top(),r=hang.top();
            lie.pop();hang.pop();
            lie.push(t-n*p);hang.push(r-m*p);
        }
		return 0;
	}
}
signed main() {
    ...
    for(int i=0;i<=k;i++) ans=max(ans,h[i]+l[k-i]-1ll*p*i*(k-i));
	write(ans);
	return signed(0);
}