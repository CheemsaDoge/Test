 ...
 * @LastEditTime: 2023-11-16 11:46:45
 * @FilePath: \TEST\面向tg选手的DP练习题\P1282 多米诺骨牌.cpp
 * @Forgive me.
...
int n;
int f[MAXN],g[MAXN];
int maxx[MAXN],a[MAXN],b[MAXN],minn[MAXN],sum;
inline void data_init() {
	read(n);
    for(int i=1;i<=n;i++) {
        read(a[i],b[i]);
		sum+=a[i]+b[i];
        maxx[i]=maxx[i-1]+max(a[i],b[i]);
		minn[i]=minn[i-1]+min(a[i],b[i]);
    }
}
...
inline void solve() {
	...
	for(int i=2;i<=n;i++) {
		...
		for(int j=minn[i];j<=maxx[i];j++) {
			int p=INF,q=INF;
			if(j>=b[i]) p=f[j-b[i]]+1;
			if(j>=a[i]) q=f[j-a[i]];
			g[j]=min(p,q);
		}
		for(int j=1;j<=maxx[i];j++) f[j]=g[j];
	}
	for(int i=0;i<=sum;i++) {
