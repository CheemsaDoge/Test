/*
 * @Author: CheemsaDoge
 * @Date: 2023-10-31 08:21:05
 * @LastEditors: CheemsaDoge
 * @LastEditTime: 2023-10-31 08:51:44
 * @FilePath: \TEST\2023.10.31\Cssgcd.cpp
...
signed main() {
	// _File();
	read(n);
	for(int i=1;i<=n;i++) read(a[i]),totgcd=__gcd(totgcd,a[i]);
	if(totgcd!=1) for(int i=1;i<=n;i++) a[i]/=totgcd;
	for(int i=1;i<=n;i++) {
		LL nowgcd=a[i];
		hash[nowgcd*totgcd]++;
		for(int j=i+1;j<=n;j++) {
			nowgcd=__gcd(nowgcd,a[j]);
			if(nowgcd==1) {
				hash[totgcd]+=n-j+1;break;
			} else hash[nowgcd*totgcd]++;
		}
	}
	read(q);
	for(int i=1;i<=q;i++) {
