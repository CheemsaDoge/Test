/*
 * @Author: CheemsaDoge
 * @Date: 2023-10-31 07:44:57
 * @LastEditors: CheemsaDoge
 * @LastEditTime: 2023-10-31 09:39:35
 * @FilePath: \TEST\2023.10.31\[HEOI2015] 定价.cpp
...
signed main() {
	...
	while(t--) {
		...
		for(LL i=l;i<=r;) {
			...
			LL tmp2=tmp%10,tot=0,t;t=tmp2==5?-1:0;
			while(tmp) tmp/=10,tot++;
			if(2*tot+t<ans) ans=2*tot+t,anst=i;
			i+=pow10[cnt];
			if(ans==1) break;
		}
		write(anst,'\n');
